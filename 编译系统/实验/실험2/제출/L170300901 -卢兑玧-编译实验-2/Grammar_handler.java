...
public class Grammar_handler {
	...
	public void readGrammarFromFile() {
		try {
			BufferedReader bufReader = new BufferedReader(new FileReader(new File("E:/2020春/compiler/lab2/src/grammar.txt")));
			String production;
	...
	public void getPredictionTable() {
		try {
			BufferedWriter bufWriter = new BufferedWriter(new FileWriter(new File("prediction_table.txt")));
			for (Production production : productions) {
				for (String selectSetElement : production.selectSet) {
