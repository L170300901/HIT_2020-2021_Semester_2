...
public class Parser {
	...
	// ------------------------------Construct function------------------------------
	public Parser(List<String[]> inputCache) {
	...
	public static void main(String[] args) {
		...
		Parser test = new Parser(scanning.getToken_Parser_Input());
		test.PDA();
		...
		while (stack.size() > 0 && inputCache.size() > 0) {
			...
			if (inputCache.get(0)[0].equals(stack.get(stack.size() - 1)[0])) {
				...
				// 不能在预测分析表中找到匹配的产生式的话，报错
					...
					System.out.println(stack.get(stack.size() - 1)[0] + "无法通过文法规则转换成" + inputCache.get(0)[0]);
					inputCache.remove(0);
				}
			}
		}
